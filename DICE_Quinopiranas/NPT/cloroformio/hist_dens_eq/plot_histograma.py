...
dados_mol1 = np.loadtxt('IA_cloro_EQ_D_nmove.dat')
nmove_mol1 = dados_mol1[:, 0]
dens_mol1 = dados_mol1[:, 1]
                    
dados_mol2 = np.loadtxt('IB_cloro_EQ_D_nmove.dat')
nmove_mol2 = dados_mol2[:, 0]
dens_mol2 = dados_mol2[:, 1]

dados_mol3 = np.loadtxt('IC_cloro_EQ_D_nmove.dat')
nmove_mol3 = dados_mol3[:, 0]
dens_mol3 = dados_mol3[:, 1]

dados_mol4 = np.loadtxt('ID_cloro_EQ_D_nmove.dat')
...
num_bins = 60
n, bins, patches = ax[0, 1].hist(dens_mol2, num_bins, density=True, facecolor='#008080', alpha=0.55)
y = norm.pdf(bins, mu, sigma)
...
num_bins = 60
n, bins, patches = ax[1, 0].hist(dens_mol3, num_bins, density=True, facecolor='#B22222', alpha=0.55)
y = norm.pdf(bins, mu, sigma)
ax[1, 0].plot(bins, y, '--', linewidth=1.5, color='black')
ax[1, 0].set_title('IC - Clorofórmio', fontsize = 'large',
                   fontweight = 'bold')
...
num_bins = 60
n, bins, patches = ax[1, 1].hist(dens_mol4, num_bins, density=True, facecolor='#ff0000', alpha=0.55)
y = norm.pdf(bins, mu, sigma)
ax[1, 1].plot(bins, y, '--', linewidth=1.5, color='black')
ax[1, 1].set_title('ID - Clorofórmio', fontsize = 'large',
                   fontweight = 'bold')


fig.text(0.52,                   # Ordena Posição x
         0.039,                   # Ordena Posição y
         'Densidade (g/cm³)',    # Texto A ser colocado
